...
const CallView: React.FC<CallViewProps> = ({ call, state, onHangup }) => {
  ...
  const getStatusColor = () => {
    switch (state) {
      ...
      case CallState.INCOMING_ASTERISK: return 'text-purple-400';
  ...
  const getStatusText = () => {
    switch (state) {
      ...
      case CallState.INCOMING_ASTERISK: return 'INCOMING FROM ASTERISK';
      case CallState.OUTGOING_GSM: return 'DIALING GSM...';
      case CallState.OUTGOING_ASTERISK: return 'ORIGINATING TO ASTERISK...';
  ...
  return (
    <div className={`bg-[#0a0a0a] border border-white/10 rounded-2xl p-6 relative overflow-hidden transition-all duration-300 ${state === CallState.BRIDGING ? 'shadow-[0_0_30px_rgba(34,197,94,0.1)]' : ''}`}>
      ...
      {(state === CallState.INCOMING_GSM || state === CallState.INCOMING_ASTERISK) && (
      ...
      <div className="flex items-center justify-between gap-3 mt-6 relative z-10">
        ...
        <div className="flex flex-col items-center shrink-0 opacity-50">
          {call?.direction === 'GSM_TO_ASTERISK' ? (
        ...
        <div className="flex-1 bg-white/5 p-3 rounded-xl border border-white/5 text-right backdrop-blur-sm">
          <p className="text-[8px] text-gray-500 font-black uppercase mb-1 tracking-widest">ASTERISK</p>
          <p className="text-xs font-mono font-bold truncate text-gray-200">{call?.asteriskChannel || '---'}</p>
